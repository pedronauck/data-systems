...
pub fn parse_fn(input: &DeriveInput, field_names: &[&Ident]) -> TokenStream {
    ...
    let parse_fields = field_names.iter().map(|name| {
        quote! {
            let #name = pedronauck_streams_subject::subject::parse_param(&self.#name);
        }
    });
...
pub fn schema_fn(
        ...
        field_names
            ...
            .map(|(name, ty)| {
                ...
                quote! {
                    fields.insert(
                        #name_str.to_string(),
                        pedronauck_streams_subject::subject::FieldSchema {
    ...
    quote! {
        fn schema(&self) -> pedronauck_streams_subject::subject::Schema {
            let mut fields = pedronauck_streams_subject::subject::IndexMap::new();
            #(#field_entries)*

            pedronauck_streams_subject::subject::Schema {
                id: #id.to_string(),
